...
// 🎯 ماژول ساعت محلی زنده و تشخیص هوشمند پرچم کشور کاربر بر اساس زبان سیستم/منطقه زمانی
function initClockAndGeoFlag() {
    // بروزرسانی زنده ساعت
    setInterval(() => {
        const now = new Date();
        const hrs = String(now.getHours()).padStart(2, '0');
        const mins = String(now.getMinutes()).padStart(2, '0');
        document.getElementById('liveClock').innerText = `${hrs}:${mins}`;
    }, 1000);

    // تشخیص پرچم موقعیت
    try {
        const locale = navigator.language || 'fa-IR';
        const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || '';
        let flag = '🏳️';

        if (locale.includes('IR') || timeZone.toLowerCase().includes('tehran')) {
            flag = '🇮🇷';
        } else if (locale.includes('US') || timeZone.toLowerCase().includes('america')) {
            flag = '🇺🇸';
        } else if (locale.includes('GB') || timeZone.toLowerCase().includes('london')) {
            flag = '🇬🇧';
        } else if (locale.includes('DE') || timeZone.toLowerCase().includes('berlin')) {
            flag = '🇩🇪';
        } else {
            // استخراج کد دو رقمی کشور از روی لوکال سیستم به عنوان فال‌بک
            const countryCode = locale.split('-')[1];
            if (countryCode && countryCode.length === 2) {
                flag = countryCode.toUpperCase().replace(/./g, char => String.fromCodePoint(char.charCodeAt(0) + 127397));
            }
        }
        document.getElementById('userFlag').innerText = flag;
    } catch (e) { console.log("Error finding country flag:", e); }
}
...
function renderCategoriesMenu() {
    ...
    mainCategories.forEach(cat => {
        ...
        grid.innerHTML += `
            <div onclick="navigateTo('gallery', '${cat.id}')" class="relative h-32 sm:h-36 rounded-2xl overflow-hidden cursor-pointer group bg-zoneBgMuted border border-white/5 hover:scale-[1.01] transition-all duration-200">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-zoneBg via-transparent to-transparent flex flex-col justify-end p-4">
...
function renderExplorerTree() {
    ...
    const latestActive = (currentCategory === 'latest') ? 'bg-zoneAccent text-white font-black shadow-md' : 'text-gray-400 hover:bg-white/5';
    treeContainer.innerHTML += `
    ...
    mainCategories.forEach(cat => {
        ...
        if (subs.length > 0) {
            subItemsHtml = `<div class="submenu-transition ${isOpen ? 'submenu-open' : ''} flex flex-col pr-3 my-1 border-r border-white/5 mr-2 gap-0.5">`;
            subs.forEach(sub => {
        ...
        treeContainer.innerHTML += `
            <div class="flex flex-col">
                <div class="flex items-center justify-between px-3 py-2 rounded-xl text-xs cursor-pointer transition-all ${catClass}">
                    ...
                    ${subs.length > 0 ? `
                    <div onclick="toggleTreeFolder(event, '${cat.id}')" class="p-1 hover:bg-white/10 rounded-md transition-all">
    ...
    for(let i=0; i<6; i++) {
        grid.innerHTML += `
            <div class="flex flex-col gap-3 relative pt-4 opacity-70">
                <div class="skeleton-block w-44 h-9 mx-auto rounded-full"></div>
                <div class="skeleton-block w-full h-[210px] rounded-[1.5rem]"></div>
                <div class="skeleton-block w-full h-10 rounded-xl mt-1"></div>
...
function renderCards(mods) {
    ...
    mods.forEach((mod) => {
        ...
        cardBox.innerHTML = `
            <div class="absolute top-[-26px] left-1/2 -translate-x-1/2 z-20 bg-slate-950 text-white border-2 border-indigo-500/60 px-7 py-2 rounded-full text-xs sm:text-sm font-black shadow-2xl tracking-wide whitespace-nowrap min-w-[160px] text-center">
                ${mod.title}
            ...
            <div class="mt-0.5 flex flex-col gap-2 px-1">
                <div class="flex justify-between items-center bg-zoneBgMuted/60 backdrop-blur-md px-3 py-2.5 rounded-xl text-[11px] text-gray-400 shadow-sm border border-white/5">
                    <div class="flex items-center gap-4 font-bold">
                        <span><i class="fas fa-hdd text-zoneGlow ml-1"></i>${mod.size}</span>
                        <span class="text-indigo-400"><i class="fas fa-user-shield ml-1"></i>${mod.adminName}</span>
                    </div>
            ...
            <div class="w-full flex gap-1.5">
                <button onclick="openInfoModal(event, ${trueGlobalIndex})" class="w-10 h-10 bg-zoneBgMuted hover:bg-white/10 border border-white/5 rounded-xl flex items-center justify-center text-gray-400 hover:text-white text-xs transition-all active:scale-95">
                    <i class="fas fa-info"></i>
                </button>
                
                <button onclick="copyPassword(event, '${mod.password}')" class="w-10 h-10 bg-zoneBgMuted hover:bg-white/10 border border-white/5 rounded-xl flex items-center justify-center text-amber-500 text-xs transition-all active:scale-95" title="Copy Password">
                ...
                <button onclick="swapCardImages(${trueGlobalIndex})" class="w-10 h-10 bg-zoneBgMuted hover:bg-white/10 border border-white/5 rounded-xl flex items-center justify-center text-indigo-400 text-xs transition-all active:scale-95">
...
function swapCardImages(cardIdx) {
    ...
    }, 750); // همگام با تایم‌اوت ترنزیشن CSS جدید
...
function openLeaderboard() {
    ...
    sortedAdmins.forEach((admin, idx) => {
        let badge = `<span class="text-xs font-bold text-gray-400 bg-white/5 w-6 h-6 rounded-full flex items-center justify-center border border-white/5">${idx + 1}</span>`;
        ...
        listContainer.innerHTML += `
            <div class="flex items-center justify-between p-2.5 bg-white/5 rounded-xl border border-white/5">
